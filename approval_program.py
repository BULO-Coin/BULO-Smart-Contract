def approval_program():
    ...
    on_setup = Seq(
        If(
            ...
            Seq(
                ...
                #The Bulo id is a constant, fixed outside of the contract logic
                #TODO: change that
                App.globalPut(Bytes("BULO_ID"), Int(int(BULOid))),
                InnerTxnBuilder.Begin(),
                InnerTxnBuilder.SetFields(
                    {
                        TxnField.type_enum: TxnType.AssetTransfer,
                        TxnField.xfer_asset: Int(int(BULOid)),
                ...
                #The RUGPULLid id is a constant, fixed outside of the contract logic
                #TODO: change that
                App.globalPut(Bytes("Buloed_ASA_ID"), Int(int(RUGPULLid))),
                InnerTxnBuilder.Begin(),
                InnerTxnBuilder.SetFields(
                    {
                        TxnField.type_enum: TxnType.AssetTransfer,
                        TxnField.xfer_asset: Int(int(RUGPULLid)),
                        TxnField.asset_amount: Int(0),
                        TxnField.asset_receiver: Global.current_application_address(),
                    }
                ),
    ...
    I_Promise_I_will_not_Rugpull = Seq(
        ...
        If(
            ...
            Seq(
                ...
                #We setup the upper limit for the liquidity, to use in conjusction with the lock_until date to know if the coin is successfull
                #TODO: implement the lock_until date check and the Did_we_succeed function
    ...
    def ComputeBuloAmmount(amount_of_shitcoins: Expr):
        ...
        return Seq(
            ...
            If(
                #We check the provided asset is indeed the Buloed ASA to avoid getting tinymanned
    ...
    Did_we_succeed = Seq(
        #Not yet implemented, that's so sad
        Approve(),
    )    
